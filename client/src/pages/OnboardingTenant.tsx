...
export default function OnboardingTenant() {
  ...
  if (!isAuthenticated) return <div className="grid min-h-screen place-items-center bg-[#f8f7f4] px-5"><div className="max-w-md rounded-3xl border border-[#dce5df] bg-white p-8 text-center shadow-sm"><ShieldCheck className="mx-auto h-9 w-9 text-[#5b8f74]" /><h1 className="mt-4 font-display text-3xl text-[#1e3742]">Entre para criar seu ambiente</h1><p className="mt-3 text-sm leading-6 text-[#697980]">A criação da empresa fica vinculada a uma identidade autenticada e auditável.</p><Button className="mt-6 rounded-full bg-[#1c3542] text-white" onClick={() => startLogin()}>Entrar com segurança <ArrowRight className="ml-2 h-4 w-4" /></Button></div></div>;
