...
export default function ErpPage() {
  ...
  const documents = trpc.erp.lookupByCnpj.useQuery(
    { tenantId: tenantId ?? 0, cnpj },
  ...
  const submit = (event: FormEvent) => {
    ...
    if (cnpj.replace(/\D/g, "").length !== 14) return toast.error("Informe um CNPJ com 14 dígitos.");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-2 sm:p-5">
      <section className="rounded-[1.7rem] border border-[#dfe9e2] bg-[#f3f8f4] p-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[#687980]">Quando sua operação usar NetSuite, consulte registros e documentos pelo CNPJ autorizado. Os PDFs gerados ficam no armazenamento privado da empresa.</p>
        <form onSubmit={submit} className="mt-6 flex max-w-xl gap-2">
          <Input value={cnpj} onChange={event => setCnpj(event.target.value)} placeholder="CNPJ da empresa ou cliente" inputMode="numeric" />
      ...
      <section className="overflow-hidden rounded-3xl border border-[#dfe8e2] bg-white">
        <div className="border-b border-[#edf1ee] px-6 py-5">
          ...
          <p className="mt-1 text-xs text-[#78878d]">A resposta depende das permissões OAuth e do mapeamento de CNPJ configurado para o NetSuite deste tenant.</p>
        </div>
