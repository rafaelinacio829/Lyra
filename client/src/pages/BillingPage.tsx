...
import { Boxes, Check, CreditCard, ExternalLink, Loader2, Sparkles } from "lucide-react";
...
export default function BillingPage() {
  ...
  const [addonQuantities, setAddonQuantities] = useState<Record<string, number>>({ members: 1, agents: 1, messages: 1 });
  ...
  const addonCatalog = trpc.billing.addonCatalog.useQuery({ tenantId: tenantId ?? 0 }, { enabled: Boolean(tenantId) });
  const addons = trpc.billing.addons.useQuery({ tenantId: tenantId ?? 0 }, { enabled: Boolean(tenantId) });
  const addonCheckout = trpc.billing.createAddonCheckout.useMutation({ onSuccess: result => { toast.success("Abrindo checkout do pacote adicional."); window.open(result.url, "_blank", "noopener,noreferrer"); }, onError: error => toast.error(error.message) });
  ...
  return <div className="mx-auto max-w-6xl space-y-6 p-2 sm:p-5">
    ...
    <section className="rounded-3xl border border-[#dce7df] bg-white p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#5f9477]">Capacidade sob demanda</p><h2 className="mt-2 font-display text-2xl text-[#2d4752]">Amplie sua operação sem trocar de plano.</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-[#78878d]">Cada pacote é recorrente e passa a valer após a confirmação do pagamento. Os novos limites são aplicados diretamente na operação.</p></div><Boxes className="h-6 w-6 shrink-0 text-[#5c9074]" /></div><div className="mt-5 grid gap-3 lg:grid-cols-3">{addonCatalog.data?.map(item => { const quantity = addonQuantities[item.type] || 1; const active = addons.data?.filter(addon => addon.type === item.type && addon.status === "active").reduce((sum, addon) => sum + addon.quantity, 0) || 0; return <article key={item.type} className="rounded-2xl border border-[#e0e9e2] bg-[#f8fbf8] p-4"><p className="font-bold text-[#38535b]">{item.label}</p><p className="mt-1 min-h-10 text-xs leading-5 text-[#74858a]">{item.description}</p><p className="mt-4 font-display text-2xl text-[#29434e]">{(item.monthlyCents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}<span className="ml-1 font-sans text-xs font-normal text-[#77878d]">por {item.unitLabel}/mês</span></p><div className="mt-4 flex items-center gap-2"><Input aria-label={`Quantidade de ${item.label}`} type="number" min="1" max="100" value={quantity} onChange={event => setAddonQuantities({ ...addonQuantities, [item.type]: Math.max(1, Number(event.target.value)) })} className="h-9 w-16 rounded-xl bg-white text-center" /><Button size="sm" disabled={addonCheckout.isPending || current.billingMethod !== "stripe"} onClick={() => addonCheckout.mutate({ tenantId, type: item.type, quantity })} className="flex-1 rounded-full bg-[#21404b] text-white hover:bg-[#315967]">Comprar</Button></div><p className="mt-3 text-[11px] text-[#678177]">Ativo hoje: {active} {item.unitLabel}{active === 1 ? "" : "s"}</p>{current.billingMethod !== "stripe" && <p className="mt-2 text-[11px] text-[#a36c50]">Solicite este pacote ao administrador da conta.</p>}</article>; })}</div></section>
