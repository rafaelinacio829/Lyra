...
export const aiProviderCatalog: Record<AiProviderId, { label: string; kind: "direct" | "orchestrator" | "visual" | "custom"; defaultBaseUrl: string; defaultModel: string; credentialLabel: string; description: string }> = {
  ...
  adk: { label: "Google ADK (endpoint publicado)", kind: "orchestrator", defaultBaseUrl: "", defaultModel: "", credentialLabel: "Token do endpoint ADK", description: "Endpoint HTTP publicado por um agente Google ADK; o Lyra envia mensagem e contexto do contato." },
