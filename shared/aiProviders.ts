export const aiProviderIds = ["dify", "openai", "anthropic", "gemini", "adk", "langgraph", "flowise", "langflow", "n8n", "native", "other"] as const;
...
export const aiProviderCatalog: Record<AiProviderId, { label: string; kind: "direct" | "orchestrator" | "visual" | "custom"; defaultBaseUrl: string; defaultModel: string; credentialLabel: string; description: string }> = {
  ...
  adk: { label: "Google ADK (endpoint publicado)", kind: "orchestrator", defaultBaseUrl: "", defaultModel: "", credentialLabel: "Token do endpoint ADK", description: "Endpoint HTTP publicado por um agente Google ADK; o Flow One envia mensagem e contexto do contato." },
  ...
  native: { label: "Agente nativo", kind: "direct", defaultBaseUrl: "", defaultModel: "", credentialLabel: "Credencial do provedor", description: "Implementação nativa do Flow One para evolução futura." },
