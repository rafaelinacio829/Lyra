...
import { requirePlatformAdmin } from "../tenantAccess";
...
export const platformRouter = router({
  ...
  setTenantStatus: protectedProcedure.input(z.object({ tenantId: z.number().int().positive(), status: z.enum(["active", "suspended"]) })).mutation(async ({ ctx, input }) => {
    requirePlatformAdmin(ctx.user.role);
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco de dados indisponível." });
