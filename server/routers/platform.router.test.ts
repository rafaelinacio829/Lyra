...
describe("procedures de plataforma", () => {
  ...
  it("retorna clientes e métricas comerciais ao super-admin", async () => {
    ...
    getDb.mockResolvedValue({
      select: vi.fn(() => ({
        from: () => ({ leftJoin: () => ({ leftJoin: () => ({ orderBy: () => Promise.resolve(rows) }) }) }),
      })),
    });
    await expect(platformRouter.createCaller(adminContext).overview()).resolves.toMatchObject({ customers: rows, metrics: { totalCustomers: 1, activeCustomers: 1, mrrCents: 29900 } });
  });
});
