...
describe("integrations.whatsappChannels", () => {
  ...
  it("agrega métricas por conexão e marca o canal padrão", async () => {
    getDb.mockResolvedValue({ select: vi.fn(() => chain([{ defaultWhatsAppIntegrationId: 21 }])), execute: vi.fn().mockResolvedValue([{ id: 21, provider: "meta", name: "Vendas", status: "active", lastVerifiedAt: new Date("2026-08-01"), lastError: null, totalConversations: "12", pendingConversations: "3", avgFirstResponseMinutes: "4.5", lastActivityAt: new Date("2026-08-16") }]) });
    ...
    expect(result.channels[0]).toMatchObject({ id: 21, name: "Vendas", isDefault: true, totalConversations: 12, pendingConversations: 3, avgFirstResponseMinutes: 4.5 });
  });
