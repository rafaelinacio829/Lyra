...
export function prepareOperationalAlerts(input: OperationalAlertInput): PreparedOperationalAlert[] {
  ...
  if (input.unassignedConversations > 0) alerts.push({ kind: "unassigned", subject: "Flow One: conversas sem atendente", body: `${input.unassignedConversations} conversa(s) humana(s) aguardam responsável.` });
  if (input.slaRiskConversations > 0) alerts.push({ kind: "sla", subject: "Flow One: risco de SLA", body: `${input.slaRiskConversations} conversa(s) humana(s) estão sem primeira resposta há mais de 20 minutos.` });
  if (input.trialEndsAt && input.trialEndsAt.getTime() > now.getTime() && input.trialEndsAt.getTime() - now.getTime() < 3 * 24 * 60 * 60 * 1000) alerts.push({ kind: "trial", subject: "Flow One: fim do período de teste", body: `O trial termina em ${input.trialEndsAt.toLocaleDateString("pt-BR")}.` });
  return alerts;
}
...
export function prepareWeeklySummary(input: WeeklySummaryInput): { subject: string; body: string } {
  ...
  return { subject: `Flow One: resumo semanal de ${input.tenantName}`, body: `Resumo operacional de ${input.tenantName}: ${input.receivedMessages} mensagem(ns) recebida(s), ${input.resolvedConversations} conversa(s) resolvida(s), ${input.awaitingHuman} conversa(s) aguardando atendimento humano e primeira resposta média de ${response}.` };
}
